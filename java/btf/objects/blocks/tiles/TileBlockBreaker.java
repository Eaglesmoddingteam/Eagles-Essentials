...
public class TileBlockBreaker extends TileEntity {
	...
	public void scan() {
		if(!initialized)
			...
			breakingstate = world.getBlockState(breaking);
			if(breakingstate.getBlock() != Blocks.AIR && breakingstate.getBlock() != Blocks.BEDROCK
				...
				if (!world.isRemote) {
					...
					for (ItemStack drop: drops) {
						...
						if(world.getTileEntity(behind).hasCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null)) {
							IItemHandler ItemHandler = world.getTileEntity(behind).getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null);
							...
							for(int i = 0; i <= ItemHandler.getSlots(); i++) {
									if(ItemHandler.getStackInSlot(i).isEmpty()) {
										...
										spawned = true;
										break;
									} else if(ItemHandler.getStackInSlot(i).getItem() == stack.getItem() && (ItemHandler.getStackInSlot(i).getCount() + stack.getCount()) <= 64) {
										ItemHandler.insertItem(i, stack, false);
										spawned = true;
										break;
									}
